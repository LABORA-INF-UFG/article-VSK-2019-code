...
path = "/media/arquivos/Dropbox/mestrado/3-semestre/dissertacao/frameworkdeeventos/gerador-graficos/data/MovementsInterval.csv"
intervals.readed = read.table(path, sep=",", dec=".", header=FALSE)
dim(intervals.readed)

head(intervals.readed, 10)
#2902 + 3152 + 1918 + 1938

operations <- subset(intervals.readed, 
                     !(intervals.readed[[2]] %in% c("wheelin-wheelout", "wheelout-wheelin")))

think.time <- as.numeric(operations[[1]])
think.time <- think.time[!is.na(think.time)]

(x <- subset(operations, operations[[1]]  < 10))
dim(x)
...
q01
intervals <- think.time#[think.time > q01]
intervals <- intervals#[intervals < q99]

length(think.time[think.time <= 500]) / length(think.time)
...
# x.pareto <- rpareto(n=length(intervals), pareto.params$xmin, pareto.params$exponent)
